...
\veb@asset.fit@ can show model fit of each assets, for example for asset \verb@AA@. 
<<single.asset.fit>>=
summary(fit.time$asset.fit$AA)
@
shows only beta of SP500 is significant.  

\verb@fitTimeSeriesFactorModel@ also have various variable selection algorithm to choose. One can include all the factor and let the function to decide which one is the best model. For example, we inlcude all common factors and use method \verb@stepwise@ which utilizes \verb@step@ function in \verb@stat@ package 
...
Generic functions like \verb@summary@, \verb@print@, \verb@plot@ and \verb@predict@ can be used for Time Series factor model as well like previous section.
