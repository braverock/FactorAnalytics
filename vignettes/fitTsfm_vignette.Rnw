...
Setting \code{plot.single=TRUE} enables individual asset plots. If there is more than one asset fit by the fitted object \code{x}, \code{asset.name} is also necessary. However, if \code{x} contains only one asset's factor model fit, \code{plot.tsfm} can infer \code{asset.name} without user input. 
...
Remarks:
\begin{itemize}
\item CUSUM plots (individual asset plot options 13, 14 and 15) are applicable 
only for \code{fit.method="OLS"}.
\item Rolling estimates (individual asset plot option 16) is not applicable for 
