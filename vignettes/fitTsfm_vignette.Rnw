...
Remarks:
\begin{itemize}
\item CUSUM plots (individual asset plot options 14, 15 and 16) are applicable 
only for \code{fit.method="LS"}.
\item Rolling estimates (individual asset plot option 17) is not applicable for 
