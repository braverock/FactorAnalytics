...
#' remaining assets. VaR used in ES computation is computed as an estimated
...
## among remaining assets. VaR used in ES computation is computed as an estimated 
## quantile using the Cornish-Fisher expansion
## inputs:
...
## 
  ...
  ## use in RAM object
    ...
    pVaR = as.numeric(VaR(r.p[], p=(1-tail.prob),method="modified"))
    idx = which(r.p[] <= pVaR)
    pES = -mean(r.p[idx]) 
    temp.w = w
    iES = matrix(0, n.w, 1)
    ...
    ## set weight for asset i to zero and renormalize
      ...
      pVaR.new = as.numeric(VaR(temp.r.p[], p=(1-tail.prob),method="modified"))   
