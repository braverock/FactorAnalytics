...
fitFfmDT <- function(ffMSpecObj,
	...
	if(!ffMSpecObj$model.MSCI){
		...
		if ((length(ffMSpecObj$exposures.char) >1) && !addIntercept) {
		...
		if (length(ffMSpecObj$exposures.char) && !ffMSpecObj$model.MSCI){
	...
	else if(ffMSpecObj$model.MSCI){
		
		asset.names <- unique(ffMSpecObj$asset.var)
		...
		time.periods <- unique(ffMSpecObj$dataDT[[ffMSpecObj$date.var]])
		...
		if (length(ffMSpecObj$exposures.char)) {
			...
			{
				ffMSpecObj$dataDT[, eval(i):= as.factor(ffMSpecObj$dataDT[,get(i)]),with=TRUE]
				if (grepl("SECTOR",toupper(i))) 
					formula.ind = as.formula(paste(ffMSpecObj$yVar, "~", i, "-1"))
				else formula.cty = as.formula(paste(ffMSpecObj$yVar, "~", i, "-1"))
			}
		}
		...
		beta <- model.matrix(fm.formula, data=ffMSpecObj$dataDT)
		beta.ind <- model.matrix(formula.ind, data=ffMSpecObj$dataDT)
		beta.cty <- model.matrix(formula.cty, data=ffMSpecObj$dataDT)
		beta.mic <- cbind("Market" = rep(1, nrow(beta.ind)), beta.ind, beta.cty)
		if(length(ffMSpecObj$exposures.num) > 0)
			beta.style<- beta[,ffMSpecObj$exposures.num]
		
		fac.names.indcty = lapply(seq(ffMSpecObj$exposures.char), function(x)
					paste(levels(ffMSpecObj$dataDT[,get(ffMSpecObj$exposures.char[x])]),sep=""))
		if(grepl("SECTOR", toupper(ffMSpecObj$exposures.char[1]))){
			factor.names <- c("Market",unlist(fac.names.indcty),
					ffMSpecObj$posures.num)
		}else{
			factor.names <- c("Market", unlist((fac.names.indcty)[2]),unlist((fac.names.indcty)[1]),
					ffMSpecObj$exposures.num)
		}
		
		asset.names <- unique(ffMSpecObj$dataDT[[ffMSpecObj$asset.var]])
		
		rownames(beta.mic) <- rep(asset.names, TP)
		N <- length(asset.names)
		#Define Return matrix 
		returns = matrix(ffMSpecObj$dataDT[[ffMSpecObj$yVar]],nrow = N)
		...
		fmMSCI.formula = as.formula(paste(ffMSpecObj$yVar, "~", "B.mod+", paste(ffMSpecObj$exposures.num, collapse="+"),"-1" ))                 
		reg.list <- by(data=ffMSpecObj$dataDT, INDICES=ffMSpecObj$dataDT[[ffMSpecObj$date.var]], 
				FUN=function(x) {lm(data=x, formula=fmMSCI.formula, 
							na.action=na.fail)})
		#Find weights for WLS regression
		if (grepl("W",fit.method)) {
			if (ffMSpecObj$rob.stats) {
			...
			{ #Extract Residuals
				...
				ffMSpecObj$dataDT<- cbind(ffMSpecObj$dataDT, W = 1/as.numeric(w))
			}
			else
			{
				ffMSpecObj$dataDT <- cbind(ffMSpecObj$dataDT, W=1/resid.var)
			}
		...
		names(reg.list) = as.character(unique(ffMSpecObj$dataDT[[ffMSpecObj$date.var]]))
		...
		reg.listDT =  data.table()
		reg.listDT[,DateYearMon:=names(reg.list)]
		reg.listDT[,reg.list:=reg.list]
		reg.listDT[,id:=paste0(unique(ffMSpecObj$dataDT[,get(a_)]),collapse=",")]
		
		if(grepl("W",fit.method))
			reg.listDT[,w:=apply(w,2,paste0,collapse=",")]
...
extractRegressionStats <- function(specObj, fitResults, full.resid.cov=FALSE){
	...
	if(!specObj$model.MSCI){
		...
		residuals1$date = as.Date(residuals1$date,format="%Y-%m-%d")
		...
		if (specObj$addIntercept == FALSE || specObj$model.styleOnly ==TRUE) {
			...
			g.cov <- cov(g)
			K <- length(levels(specObj$dataDT[,specObj$exposures.char, with = F][[1]]))
		...
		colnames(factor.returns) = as.character(unique(ffMSpecObj$dataDT[[ffMSpecObj$date.var]]))
		if(length(ffMSpecObj$exposures.num) > 0){
			residuals = returns - B.mod %*% g[1:(K1+K2-1), ] - B.style %*% g[(K1+K2):nrow(g), ] #NxT
		}else residuals = returns - B.mod %*% g[1:(K1+K2-1), ]
		colnames(residuals) = as.character(unique(ffMSpecObj$dataDT[[ffMSpecObj$date.var]]))
		...
		names(r2) = as.character(unique(ffMSpecObj$dataDT[[ffMSpecObj$date.var]]))
